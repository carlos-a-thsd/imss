...
export const ChartLib = {
  ...
  line: (id, data, options) => {
    options = Object.assign(
      { xvalues: "date", yvalues: ["value"], axes: true, ds: {} },
      options || {}
    );
    ...
    if (_charts[id]) {
      ...
      _charts[id].options.scales.yAxis.min = options.yrange
        ? options.yrange[0]
        : undefined;
      _charts[id].options.scales.yAxis.max = options.yrange
        ? options.yrange[1]
        : undefined;
  ...
  bar: (id, data, options) => {
    ...
    if (_charts[id]) {
      ...
      _charts[id].options.scales.xAxis.ticks.callback =
        options.type == "hbar" ? xcb : ycb;
      _charts[id].options.scales.yAxis.ticks.callback =
        options.type == "hbar" ? ycb : xcb;
      const tcb = _charts[id].options.plugins.tooltip.callbacks;
      tcb.title = (tooltipItems) => {
